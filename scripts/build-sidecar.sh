...
STAGING="$(mktemp -d /tmp/ds4-sidecar.XXXXXX)"
...
cp "$DS4_SOURCE/ds4-server" "$STAGING/ds4-server"

# Build optimized variant (native CPU flags) for M4+ Macs.
make -C "$DS4_SOURCE" clean
make -C "$DS4_SOURCE" ds4-server
cp "$DS4_SOURCE/ds4-server" "$STAGING/ds4-server-m4"

if [[ -z "$APP_BUNDLE" ]]; then
  echo "Built $STAGING/ds4-server"
  echo "Built $STAGING/ds4-server-m4"
...
install -m 755 "$STAGING/ds4-server" "$APP_BUNDLE/Contents/MacOS/ds4-server"
install -m 755 "$STAGING/ds4-server-m4" "$APP_BUNDLE/Contents/MacOS/ds4-server-m4"
